/**
 * grunt-cmd-transport改版，添加module依赖检测失败提醒开关
 * User: qisx
 * Date: 13-9-24
 * Time: 下午4:12
 */
exports.init = function(grunt) {
    ...
    exports.htmlParser = function (fileObj, options) {
        var data = fileObj.srcData || grunt.file.read(fileObj.src);

        var con = [
            'define("' + unixy(options.idleading + fileObj.name) + '", [], function(require, exports, module) {',
            '    module.exports="' + data.replace(/\"/g, '\\x22').replace(/\r\n|\n|\r/g, "\\n").replace(/(\u2028|\u2029)/g, '\$1') + '";',
            '});'
        ].join('\n');
        grunt.file.write(fileObj.dest + '.js', con);
    };
    ...
    var baseDir,
        getCssContent = function (src, filePath) {
            var self = arguments.callee,
                reg = /@import\s.+;/gim,
                matches = src.match(reg) || [],
                str = src.replace(/\s*@import.+;\s*/gi, ''),
                _path;

            str = str.replace(/url(?:\s+)?\(([^\)]+)\)/igm, function (match, url) {
                url = url.replace(/'|"/g, '');

                if (/^\/|about:blank|^http[s]?:\/\//.test(url)) {
                    return 'url(' + url + ')';
                }

                return 'url(' + path.relative(baseDir, path.resolve(path.dirname(filePath), url)).replace(/\\/g, '/') + ')';
            });

            if (matches.length > 0) {
                for (var i = 0; i < matches.length; i++) {
                    _path = matches[i].replace(/\s*@import\s+(url\()*['"]*(.*?)['"]*\)*;/gi, '$2');
                    _path = path.resolve(path.dirname(filePath), _path);

                    if (grunt.file.exists(_path)) {
                        str += self(grunt.file.read(_path), _path);
                    }
                }
            }
            return str;
        };
    exports.cssConcatParser = function (src, filePath, _baseDir) {
        baseDir = path.resolve(_baseDir);

        return getCssContent(src, filePath);
    };
